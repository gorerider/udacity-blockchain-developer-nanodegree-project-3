...
contract SupplyChain is FarmerRole, DistributorRole, RetailerRole, ConsumerRole, Ownable {
  ...
  function harvestItem(uint _upc, address _originFarmerID, string _originFarmName, string _originFarmInformation, string  _originFarmLatitude, string  _originFarmLongitude, string  _productNotes) public onlyFarmer
  {
    ...
    item.productID = sku; // productId will be the sku because it's unique
    item.productNotes = _productNotes;
    item.productPrice = 0; // will be set once it's on sale
    item.itemState = defaultState;
    item.distributorID = address(0); // not set yet
    item.retailerID = address(0); // not set yet
    item.consumerID = address(0); // not set yet
  ...
  function shipItem(uint _upc) public 
    ...
    verifyCaller(items[_upc].ownerID)
    {
      ...
      // items[_upc].retailerID = SKIPPED_DUE_TO__IS_MISSING_IN_SIGNATURE?!;
      // Emit the appropriate event
      emit Shipped(_upc);
  }
